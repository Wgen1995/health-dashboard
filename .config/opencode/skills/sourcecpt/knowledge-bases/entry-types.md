...
|---|-----------|---------|------------------------------|------|------|
| 1 | rest | REST/Web 框架入口 | Java: `@RequestMapping\|@GetMapping\|@PostMapping\|@PutMapping\|@DeleteMapping\|@RestController` + Struts 2.x: `extends ActionSupport\|implements Action\|extends ActionSupport.*implements Validateable` + Struts 1.x: `extends org.apache.struts.action.Action\b\|extends org.apache.struts.action.DispatchAction` Go: `router.GET\|router.POST\|gin.HandlerFunc\|http.HandleFunc` Python: `app.route\|@app.route\|flask.Flask\|@api.route\|@action.route` JS: `app.get\|app.post\|express.Router\|router.get` | `@GetMapping("/api/users/{id}")` 或 Struts `public class MyAction extends ActionSupport` | 普通 `@Bean` 注解（不是入口） |
